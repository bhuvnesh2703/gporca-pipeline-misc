With
x as (Select COB_DATE, BOOK, PRODUCT_TYPE_CODE,
...
where
    a.COB_DATE IN 
    ('2018-02-28','2018-02-27','2017-12-29','2017-09-29','2017-06-30','2017-03-31')
...
y as (Select COB_DATE, Level6, Name
, Case when Level6 = 'HFS Unhedgeable' Then sum(pv10) when Level6 = 'HFS Hedgeable' Then sum(pv10) Else sum(NET_EXPOSURE) END as RankMeasure
, sum(pv10) pv10
, sum(pv50) pv50
...
, rank() over (Partition by Level6 order by ABS(sum(RankMeasure)) Desc) AS rank
, sum(pv10) pv10
, sum(pv50) pv50
, sum(NET_EXPOSURE) NET_EXPOSURE
from y 
...
Select x.COB_DATE, x.Level6,x.Sectype,
...
, max(x.spread) as spread
, sum(x.pv01) pv01
, sum(x.pv10) pv10
, sum(x.pv50) pv50
, sum(x.pv100) pv100
, sum(x.pv200) pv200
, sum(x.NET_EXPOSURE) NET_EXPOSURE
, sum(NOTIONAL) NOTIONAL
from x left join z on (x.Level6,x.Name) = (z.Level6,z.Name)
Group by x.COB_DATE, x.Level6,x.Sectype, 
Case when coalesce(z.rank,11)<10 then 0||rank
when z.rank=10 then '10'
else '11' end,
