...
WHERE
    cob_date IN ('2017-10-04','2017-09-27') 
  and  (CCC_BUSINESS_AREA IN ('CPM TRADING (MPE)', 'CPM', 'CREDIT', 'MS CVA MNE - FID', 'MS CVA MNE - COMMOD', 'LENDING', 
                               'COMMODS FINANCING') OR
     CCC_STRATEGY IN ('MS CVA MPE - DERIVATIVES', 'MS CVA MNE - DERIVATIVES')) AND
    COUNTRY_CD_OF_RISK in ('CNH','HKG')
GROUP BY
    COB_DATE,
    PRODUCT_TYPE_CODE,
    PRODUCT_SUB_TYPE_CODE,
