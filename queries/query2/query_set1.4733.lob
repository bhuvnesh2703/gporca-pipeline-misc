...
and CONSOLIDATED_RATING not in ('AAA','AA','A','BBB')
...
Select x.COB_DATE, x.Level6, x.Product_Type_Code, x.Loan_Type, x.sectype, x.CONSOLIDATED_RATING, x.industry,
...
where x.level6 in ('HFS Unhedgeable', 'HFS Hedgeable')
Group by x.COB_DATE, x.Level6, x.Product_Type_Code, x.Loan_Type, x.sectype, x.CONSOLIDATED_RATING, x.industry,
Case when coalesce(z.rank,11)<10 then 0||rank
when z.rank=10 then '10'
else '11' end,
